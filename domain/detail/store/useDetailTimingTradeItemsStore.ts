...
interface DetailTimingTradeEntry {
  items: TradeItem[];
...
interface DetailTimingTradeItemsStore {
  entries: Record<string, DetailTimingTradeEntry>;
  ensureEntry: (key: string) => void;
  fetchEntry: (key: string, input: TimingListInput, options?: { silent?: boolean }) => Promise<void>;
}

const EMPTY_ENTRY: DetailTimingTradeEntry = {
  items: [],
  rawItems: [],
  summary: null,
  isLoading: false,
...
export function createDetailTimingTradeItemsKey(input: TimingListInput) {
...
export const useDetailTimingTradeItemsStore = create<DetailTimingTradeItemsStore>((set, get) => ({
  entries: {},
  ensureEntry: (key) => {
    if (get().entries[key]) {
      return;
  ...
  fetchEntry: async (key, input, options) => {
    ...
    try {
      ...
      console.error("Failed to load detail timing list", error);
